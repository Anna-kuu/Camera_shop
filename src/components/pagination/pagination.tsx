      ...
      <ul className="pagination__list">
        ...
        {Array.from({length: pageCount}, (it, index) => (
          <li className="pagination__item" key={`pagination-${index}`}>
            <Link onClick={() => setActivePage(PAGE_STEP + index)} className={`pagination__link ${(activePage === (PAGE_STEP + index)) ? 'pagination__link--active' : ''}`} to={`${APIRoute.Catalog2}/${PAGE_STEP + index}`}>{PAGE_STEP + index}</Link>
          </li>
        ))}
        {activePage !== pageCount &&
        <li className="pagination__item">
          <Link onClick={() => setActivePage(activePage + PAGE_STEP)}className="pagination__link pagination__link--text" to={`${APIRoute.Catalog2}/${activePage + PAGE_STEP}`}>Далее</Link>
        </li>}
      </ul>
    </div>
  );

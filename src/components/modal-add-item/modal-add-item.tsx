...
export default function ModalAddItem({selectedCamera, setIsModalAddItemActive, setIsModalAddItemSuccess}: ModalAddItemProps): JSX.Element {
  useKeydown('Escape', () => {setIsModalAddItemActive(false); document.body.style.overflow = 'scroll';});
  const dispatch = useAppDispatch();
  return (
    <ReactFocusLock>
      <div className="modal is-active">
        <div className="modal__wrapper">
          <div onClick={() => {setIsModalAddItemActive(false); document.body.style.overflow = 'scroll';}} className="modal__overlay"></div>
          <div className="modal__content">
            ...
            <div className="modal__buttons">
              <button onClick={() => {dispatch(addCameraToBasket(selectedCamera)); setIsModalAddItemActive(false); setIsModalAddItemSuccess(true);}} className="btn btn--purple modal__btn modal__btn--fit-width" type="button">
                <svg width="24" height="16" aria-hidden="true">
                  <use xlinkHref="#icon-add-basket"></use>
                </svg>Добавить в корзину
              </button>
            </div>
            <button onClick={() => {setIsModalAddItemActive(false); document.body.style.overflow = 'scroll';}} className="cross-btn" type="button" aria-label="Закрыть попап">
